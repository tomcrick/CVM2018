...
% TODO Add annotations to images. (Issue 46)

% TODO Shorten sections (Issue 42)
...
% TODO Make intro more concise. (Issues 44, 43, 41, 40, 39, 34)
% Clearly note the contribution is the technique.
...
The remainder of this paper is structured as follows: Section~\ref{sec:related} outlines the current development and drive for non-traditional display shapes with tabletop systems; Section~\ref{sec:problem} identifies the issues arising from the use of different display shapes on a system;
A technique to minimise the influence of the identified issues is proposed in the \secref{sec:solution} section; in Section~\ref{sec:implementation} we present the development of the technique into a software framework.
Section~\ref{sec:observations} discusses the implications that arise
from the technique's use, with future developments and our conclusions presented in Section~\ref{sec:conclusion}.
...
% TODO Need to make it clear this is for 2D displays and in such a way that allows interventions at different points in the technique depending on the content. The additional control given developers results in a technique which uses only linear transformations - more complex transformations wouldn't allow for similar interventions. (Issue 36)
...
While the technique does has the ability to counter any \ac{DSDI}, it does have the drawback of requiring trade-offs in its implementation of which \acp{DSDI} it minimises the consequences.


% TODO Restructure to have Study + Results sections instead of discussion and observations. (Issues 47, 44, 40, 37, 33)

% TODO Use metrics in study. (Issues 45, 43, 38, 37, 35)
% - % Change in items' visibility
% - Change in layout scale
% - Total distance items are moved by the pull technique (perhaps relative to the layout scale).
...
The technique discussed in the previous section was implemented into version 2.1 of the {\emph{SynergyNet}} framework~\cite{McNaughton2017,AlAgha2010}, a multi-touch software framework intended for use with rectangular tabletop interfaces.
...
\begin{itemize}
  ...
  \end{itemize}
  \item \textbf{Collage}: 
  Users move visual content items about on top of a large background image to make a scrapbook-type collage of which they can then save a snapshot. 
    \begin{itemize}
...
\begin{figure*}[h!] 
 \centering
  ...
  \caption{The applications chosen for observation, from left to right: {\emph{XML Puzzle}}, {\emph{Collage}}, {\emph{Grid}} and {\emph{Simple Map}}.}
  \label{fig:apps}
\end{figure*}
...
The complexities arising from these two applications are important to consider due to the fact that the {\emph{Grid}} and {\emph{Simple Map}} applications bear the strongest similarities to the majority of legacy software.
...
Furthermore, it is important to note that although the technique presented in this work is a solution to the \acp{DSDI} of tabletop interfaces, it is not always guaranteed to be the best solution.
...
{is Professor of Computer Science \& Public Policy at Swansea University (UK).
