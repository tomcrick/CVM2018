...
\def\ManuscriptInfo{Manuscript received: 2018-08-08; accepted: 2018-xx-xx.}
...
Previously, the overwhelming majority of systems have utilised rectangular displays; this may soon change with the increasing nature of digital devices being ubiquitous and pervasive to facilitate frictionless human interaction.
...
Furthermore, we present a study focused on applying the technique to adapt several software applications to several different display shapes.
}
...
% TODO We assume there is no technological implementation of an attempt to resolve DSDIs (i.e. no warping applied by a device's set-up or driver). The scope of the technique is limited to displays where the output displays a region of the rectangular output, such as when a rectangular display is partially covered up or a projection occluded to make the display shape. This is in keeping with the majority of approaches to making non-rectangular displays detailed in the background section.
...
% TODO Add a paragraph highlighting the limitations of the technique - specifically that non-uniform backgrounds will need to be cropped or scaled (leaving blank regions of the background). This may be where warping comes in handy or context-aware filling (like filling the blank spaces with the average colour of the background image).
