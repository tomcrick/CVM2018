...
\address[newcastleEng]{School of Electrical Engineering and Computing, The University of Newcastle, Callaghan NSW 2308, Australia}
...
In an era of ubiquitous computing, more systems will be designed to naturally fit into their surroundings~\cite{Greenfield2006}.
...
The ability for a tabletop system to manage different display shapes enhances this strength allowing interfaces to better fit their environment.
...
This is the approach used with the software systems discussed thus far~\cite{Hansen2009,Shen2004}.
However, for software which may have a wide range of potential interface shapes this would require extra development work.
...
For example, SUPPLE~\cite{Gajos2004} is a system which can automatically adapt the layout of contents to fit the parameters of a display.
Content is arranged by the system to fit various display sizes and resolutions.
SUPPLE~\cite{Gajos2004} also has the ability to adapt its visual contents based on input type.
...
However, for visual content which may have a significant locational relation this technique could be considered unsuitable.
The order in which windows are created will influence where the technique positions them.
Therefore, the technique currently has no method to accommodate for any locational relation which are intended to exist between windows.
...
This is because the positioning of content items can imply their functional relations to the user~\cite{Constantine1999}.
...
The shapes chosen are shown in Figure~\ref{fig:study}.
...
% Application's from SynergyNet's library used - written by the primary but intended for other purposes.
% Applications chosen all had different methods of organising their visual contents and significance to their layout.
% Applications: 
% - XML Puzzle: Users pair together questions and their relevant answers. 100% randomly positioned content. No layout significance.
% - Collage: Users move visual items about on top of a large background image to make a scrapbook type collage to screenshot and save. Mix of random and a single instance of specifically positioned content. No layout relations other than the background.
% - Grid: 9 items in a grid layout which users can manipulate the orientation, scale and location of through multi-touch gestures - used to help familiarise users to large multi-touch interfaces. Bares most similarity to legacy software (maybe mention in observations section). 100% specifically positioned visual content. There is a strong layout relation between all of them.
% - Simple Map: Users supply countries in a config file and the app when started shows a world map with the specified counties' flags on top of them on the map - users can then move the flags around. All content in this application is specifically positioned. There is a strong layout relation between the flag items and the background image.
...
% vvv Trim down vvv
To inform the research presented, a structured literature review was carried out following an established protocol~\cite{Kitchenham2004}.
Though the results returned by the review were exhaustive it was evident that there has been little innovation in this area in the past.
This is likely due to the cyclical nature of dependency between display technology and its supporting software preventing the adoption of non-rectangular displays.
...
Though these pieces of research focus more on the impact of manually positioning visual content items for specific display shapes rather than the process of dynamically adapting them, they show that there is a growing interest in moving beyond rectangular displays.

With interest in, and support for, non-rectangular displays growing, the argument for methods of dynamically adapting content to any display shape becomes more apparent.
Being able to adapt to any display shape is useful for scenarios where the same software may be used with many different display shapes.
The usefulness of this ability increases where the developer may not know what display shapes their software could be used with in the future.
This is a likely scenario if interest in non-rectangular displays continues to grow as many device manufactures will likely use their own standards and designs but could want to adopt the same foundation for their supporting software.
This potential scenario strongly resembles the real-life scenario of mobile devices where many use the same underlying operating system (e.g. Android) but utilise displays of varying sizes, aspect-ratios and resolutions that the software must dynamically adapt to.

With a system capable of utilising different display shapes available, research into the effect of a display shape on the use of the system can be carried out.
