...
% TODO More discussion of these papers and note how they present guidelines which place more responsibility on software and content-designers.
...
% TODO Clearly state that the DSDIs presented are novel and that they were derived from first-hand observations on earlier attempts by the authors to manually fit content to different display shapes. Also discuss the difference between the primary and secondary DSDIs (primary - can happen when displaying content without modification on a different display shape, secondary - can happen when attempting to resolve initial DSDIs) - refer to dsdiFlow. Additionally, note the DSDIs were numbered by the order they were noted by the authors and bare no significance beyond identification. 
...
% TODO Link each stage to guidelines from ~\cite{Serrano2016} and ~\cite{Serrano2017}.

\subsection*{Virtual Projection}
\label{subsec:virtualprojection}

% TODO Trim discussion of this stage of the algorithm and cut algo:virtualProjection.

% TODO Need to note in here somewhere that rotating content is not an issue because tabletop interfaces are by definition horizontal. 
...
%-------------------------------------------------------------------------
% TODO Restructure Study to `Observations'
\section*{Study}
\label{sec:study}
...
% TODO Explain applications chosen from those available in the SynergyNet framework.
...
% TODO Image with all the application in.
...
%-------------------------------------------------------------------------
% TODO Restructure Study to `Discussion'
\section*{Observations}
\label{sec:observations}

% TODO Drop any discussion of 'randomly positioned content' with the algorithm and refer to it as content whose layout has no significance (because it is random).
...
%TODO Need to clearly state this is a solution, likely not always the maximal solution but one which can be easily automated and allow developers some control over its execution.
...
% TODO Discuss how the technique on this paper has some parallels with the guidelines presented in the ~\cite{Serrano2016} and ~\cite{Serrano2017} - however, the reasons for the differences and the novel contribution of the technique presented in this paper is that it can function with minimal input from developers - therefore less consideration needs to go into supporting different display shapes saving on time and resources + more freedom in design if not constrained at this level by additional guidelines.
...
% TODO Discuss how further investigation could take place with using the technique with more extreme shapes (maybe looking into finding limits on components like the pull content distance. Clearly state intention to carry out a study with the technique in future work.  Need to clearly state scope of the technique (i.e. visual content layout) - but state future work could involve further investigation of other issues like usability and readability (again, adding limits to elements of the technique).
...
\section*{Figures}
 ...
 \begin{figure}[h!]
 \centering
   % \includegraphics[width=1\textwidth]{DSDIFlowChart.png}
...
\begin{figure}[h!] 
 \centering
  % \includegraphics[width=1\textwidth]{MapApplication.png}
...
\begin{figure}[h!] 
 \centering
   % \includegraphics[width=1\textwidth]{TechniqueFlowchart.png}
