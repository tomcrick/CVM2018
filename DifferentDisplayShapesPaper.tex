...
% Did a structured lit survey so the collection of resources found which are considered to be relative is exhaustive.
\cite{Kitchenham2004}
% Only 2 recent pieces of work of relevance:
\cite{Serrano2016} % Fitting text to non-rectangular displays in legacy: http://dl.acm.org/citation.cfm?id=2858057
\cite{Serrano2017} % Application to legacy: http://dl.acm.org/citation.cfm?id=3025677
% These papers don't deal with dynamic adapting much but instead focus on manually adapting content for a specific display shape. It may be worth mentioning the technique presented in our paper is useful for scenarios where the same software may need to be used on many different display shapes - including some in the future which are currently unknown to the developer. This is a likely scenario given current trends where many device manufacturers will use their own standards and designs but use the same foundation for their supporting software (e.g. the many devices of different display form, size and resolution that all use android).
